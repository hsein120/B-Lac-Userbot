...
@icssbot.on(icss_cmd(pattern="بنعال القياده(?: |$)(.*)", command="kick"))
@icssbot.on(sudo_cmd(pattern="بنعال القياده(?: |$)(.*)", command="kick", allow_sudo=True))
async def kick(usr):
    if usr.fwd_from:
        return
    ...
    if not user:
        ...
        return
    icse = await eor(usr, "**نعال القياده قوي. 😂👍...**")
    try:
    ...
    if reason:
        await icse.edit(
            f"**- ❝ ⌊  تم طردة بنعال القياده** [{user.first_name}](tg://user?id={user.id})  𓆰."
        )
    else:
        await icse.edit(
            f"**- ❝ ⌊  تم طردة بنعال القياده** [{user.first_name}](tg://user?id={user.id})  𓆰."
...
@icssbot.on(icss_cmd(pattern="ت($| (.*))", command="pin"))
@icssbot.on(sudo_cmd(pattern="ت($| (.*))", command="pin", allow_sudo=True))
async def pin(msg):
    if msg.fwd_from:
        return
